...
export class QualityComponent implements OnInit {
    ...
    Open() {
        ...
        this.dep_rep_id = this.dep_res_one;
    ...
    ShowIncident() {
        ...
        this.incidentService.Quality(this.quality)
            .then((resultO: any) => {
    ...
    showINIncident() {
        ...
        this.incidentService.QualityIN(this.quality)
            .then((resultO: any) => {
                if (resultO.rows.length > 0) {
                    if (resultO.ok) {
                        this.allincident = resultO.rows;
    ...
    showOUTIncident() {
        ...
        this.incidentService.QualityOUT(this.quality)
            .then((resultO: any) => {
                if (resultO.rows.length > 0) {
                    if (resultO.ok) {
                        this.allincident = resultO.rows;
    ...
    DepartID(id_depart) {
        ...
        this.oneDeparts = [];
        this.personService.DepartID(id_depart)
            .then((result: any) => {
                if (result.ok) {
                    this.oneDeparts = result.rows;
                    this.name_depart = this.oneDeparts[0].name_depart;
                    console.log(this.oneDeparts[0].name_depart);
                } else {
                    console.log(JSON.stringify(result.error));
                }
            })
