...
export class IncidentComponent implements OnInit {
    ...
    Open() {
        ...
        this.idcard_incident = this.persons[0].idcard;
        // this.dep_rep_id = null;
        this.dep_rep_id = this.dep_rep_one;
        this.dep_res_id = 93;
        this.id_side = null;
        this.id_safety = null;
        this.id_type = null;
        ...
        this.personService.getSelectcard(this.idcard)
            .then((res: any) => {
                if (res.ok) {
                    ...
                    this.code_depart = this.persons[0].depart;
                    this.quality = this.persons[0].quality;
                    // console.log(this.varquality);
                    if (!this.quality) {
                        this.personService.selectDepartOne(this.code_depart)
                            .then((result: any) => {
                                if (result.ok) {
                                    this.departs = result.rows;
                                    this.dep_rep_one = this.departs[0].id_depart;
                                    this.dep_res_group = this.departs[0].code_depart;
                                    this.incidentService.getSelectOut(this.dep_rep_one, this.dep_res_group)
                                        .then((resultO: any) => {
                                            if (resultO.rows.length > 0) {
                                                if (resultO.ok) {
                                                    this.allincident = resultO.rows;
                                                    this.datanull = this.allincident.length;
                                                } else {
                        ...
                        this.personService.selectDepart(this.code_depart, this.quality)
                            .then((result: any) => {
                                if (result.ok) {
                                    this.departs = result.rows;
                                    this.dep_rep_one = this.departs[0].id_depart;
                                    this.dep_rep_two = this.departs[1].id_depart;
                                    this.dep_res_group = this.departs[0].code_depart;

                                    this.incidentService.getSelectOutraw(this.dep_rep_one, this.dep_rep_two, this.dep_res_group)
                                        .then((resultO: any) => {
                                            // console.log(resultO);
                                            if (resultO.rows.length > 0) {
                                                if (resultO.ok) {
                        ...
                        this.personService.selectDepartOne(this.code_depart)
                            .then((result: any) => {
                                if (result.ok) {
                                    ...
                                    this.incidentService.getSelectShowOut(this.dep_rep_one, this.dep_res_group)
                                        .then((resultO: any) => {
                                            if (resultO.rows.length > 0) {
                                                if (resultO.ok) {
                                                    this.allincident = resultO.rows;
                        ...
                        this.personService.selectDepart(this.code_depart, this.quality)
                            .then((result: any) => {
                                if (result.ok) {
                                    ...
                                    this.incidentService.getSelectShowOutraw(this.dep_rep_one, this.dep_rep_two, this.dep_res_group)
                                        .then((resultO: any) => {
                                            if (resultO.rows.length > 0) {
                                                if (resultO.ok) {
                                                    this.allincident = resultO.rows;
    ...
    addData() {
        ...
        if (this.idcard_incident
            ...
            console.log("การกรอกข้อมูลไม่ครบ");
            this.open = true;
        }
    }
    ...
    botLine(message) {
        // let message = 'ทดสอบ';
        this.incidentService.sendBotLine(message)
            .then((result: any) => {
                if (result.ok) {
                    let xx = result.rows;
                    console.log(xx);
                ...
                console.log(error);
                // this.alertService.error("ติดต่อฐานข้อมูลไม่ได้");
            })
    }
